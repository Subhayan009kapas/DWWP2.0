    ...
    const fetchUsers = async () => {
      try {
        ...
        const usersList = querySnapshot.docs.map(doc => ({
          id: doc.id, // Firestore document ID (email)
          ...doc.data()
        }));
        setUsers(usersList);
      } catch (error) {
          ...
          <tbody>
            {users.map(user => (
              <tr key={user.id} className="user-row">
                ...
                <td>
                  <span className={`servo-state ${user.servoState?.toLowerCase() || "unknown"}`}>
                    {user.servoState || "Unknown"}
                  </span>
                </td>
                <td>{user.currentUsage || "0 L"}</td>
                <td>{user.dueBill || "₹ 0"}</td>
                <td>
                  <button 
                    className="view-details-btn"
                    onClick={() => setSelectedUser(user)}
            ...
            <div className="user-info-section">
              ...
              <div className="info-grid">
                <div className="info-column">
                  <div className="info-item">
                    <span className="info-label">Aadhar Number:</span>
                    <span className="info-value">{selectedUser.aadhar || "N/A"}</span>
                  </div>
                  <div className="info-item">
                    <span className="info-label">Phone Number:</span>
                ...
                <div className="info-column">
                  <div className="info-item">
                    <span className="info-label">Servo State:</span>
                    <span className={`info-value ${selectedUser.servoState?.toLowerCase() || "unknown"}`}>
                      {selectedUser.servoState || "Unknown"}
                    </span>
                  </div>
                  <div className="info-item">
                    <span className="info-label">Due Bill:</span>
                    <span className="info-value">{selectedUser.dueBill || "₹ 0"}</span>
                  </div>
                </div>
              </div>
            </div>
